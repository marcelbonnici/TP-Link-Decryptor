...
First, we look to the WR841N's documented pinout to connect a CH341A flash programmer to the router's flash ROM to read all its contents. Then it was extracted via the following command: `flashrom -p ch341a_spi -r tp_link_wr841n_ext.bin`. 

Critically, the flash dump's boot partition reveals that a config section exists from 0x3E0000 to 0x3F0000.
